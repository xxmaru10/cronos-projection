...
export function applySceneDialogueRetention(state: SessionState): SessionState {
...
function reduceFateLegacy(state: SessionState, event: ActionEvent): SessionState {
    ...
    switch (type) {
        ...
        case "SEAT_STATE_CHANGED": {
            ...
            return {
                ...state,
                seats: seats.some((s) => s.userId === payload.userId)
                    ? seats.map((s) =>
...
// Alias para o SnapshotWorker (nome mais explícito no contexto do backend)
export function computeStateFromEvents(events: ActionEvent[], baseState?: SessionState): SessionState {
