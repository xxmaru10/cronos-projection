...
export const CREATURE_CAPS: Record<string, WodCreatureCaps> = {
  ...
  Carniçal: { npcReady: true,  generation: false, clan: false, sect: false, bloodPool: true,  disciplines: true,  vampiricHumanity: false, faith: false, gnose: false, shapeshift: false, quintessence: false, glamour: false, passion: false, startingBlood: "small" },
  ...
  // Story 171 — Mago: usa Quintessência (azul/triângulo invertido) no lugar do sangue, sem
  // clã/geração/seita, sem transformação. Humanidade e Vontade manuais (do statblock).
  Mago:     { npcReady: true,  generation: false, clan: false, sect: false, bloodPool: false, disciplines: true,  vampiricHumanity: false, faith: false, gnose: false, shapeshift: false, quintessence: true,  glamour: false, passion: false },
...
export type Weapon = {
  ...
  ammoCurrent?: number;
  ammoMax?: number;
  // Story 155 — explicit damage type (wins over the free-text regex inference) and
  // attack-pool bonus dice (may be negative; clamp −20..+20). Back-compat: undefined.
  damageType?: "B" | "L" | "A";
  bonus?: number;
};
...
export interface WodV20Character extends Character {
  systemData: WodV20Data;
}
...
export type WodTurnState = {
  ...
  // next WOD_V20_TURN_ADVANCE hops the turn back to them.
...
export function deriveHumanity(virtues: WodV20Data["virtues"]): number {
  return (virtues.consciencia?.rating ?? 0) + (virtues.autocontrole?.rating ?? 0);
}

export function deriveWillpower(virtues: WodV20Data["virtues"]): number {
...
export function getCustomLabel(sd: Partial<WodV20Data>, key: string, fallback: string): string {
...
export function attributeLabel(sd: Partial<WodV20Data>, key: string): string {
